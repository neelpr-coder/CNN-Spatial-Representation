...
def _compute_per_loc_mse_rot_samples(
    ...
    for i in range(len(y_test)):
        ...
        '''if i == 0:
            print("pred_rot[i] type/shape:", type(y_pred[i]), getattr(y_pred[i], "shape", None))
            print("true_rot[i] type/shape:", type(y_test[i]), getattr(y_test[i], "shape", None))
            print("diff type/shape:", type(diff), getattr(diff, "shape", None))
            print("diff:", diff)'''
        ...
        for model_name in model_names:
            ...
            for feature_selection in feature_selections:
                for decoding_model_choice in decoding_model_choices:
                    ...
                    # produce two plots, one for loc and one for rot.
        ...
        for feature_selections in all_feature_selections:
            ...
            for model_name in model_names:
                ...
                for decoding_model_choice in decoding_model_choices:
                    ...
                    # plot collected results.
                    # produce two plots, one for loc error, one for rot error.
